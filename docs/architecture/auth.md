...
**What may read it is as much the rule as how it is read** ([#193](https://github.com/joshstothard/3moji/issues/193)). Four things do: the Profile edit page and `saveProfileAction`, through `lib/profile-edit.ts`; `GET /api/viewer`, through `lib/viewer.ts`; and the account page `app/account/page.tsx`, through `lib/viewer.ts`, with `deleteAccountAction`, through `lib/session.ts` ([#195](https://github.com/joshstothard/3moji/issues/195), [below](#deleting-an-account)). **Shared chrome and the public Profile never do** — not the root layout, not the navbar, not `app/[handle]/page.tsx`. Anything that wraps every page and reads the session makes every page's response differ by visitor, and the Handle page is the most-read page in the product. So the navbar's signed-in indicator is a client island that asks `GET /api/viewer` after hydration, and the Profile's HTML stays the same bytes for everybody. `apps/web/eslint.config.mjs` fails lint if any of those three files, the island itself or the header search island (`components/header-search.tsx`, [#254](https://github.com/joshstothard/3moji/issues/254)) imports `next/headers`, `lib/session`, `lib/profile-edit` or `lib/viewer`, and `app/layout.test.tsx` renders the whole shell and asserts neither `headers()` nor `cookies()` is called. The indicator is described in [system-overview.md](system-overview.md#the-signed-in-indicator).
...
**Signing out ends the session, not just the cookie** ([#194](https://github.com/joshstothard/3moji/issues/194)). `signOutFormAction` in `components/sign-out-action.ts` calls Better Auth's `auth.api.signOut` server-side with the request's own headers: it deletes the session row the cookie names and expires the cookie, and `nextCookies()` writes that expiry onto the action's response. Only the session the request carries ends — the Account's other sessions, in another browser or on another device, stay signed in. Both halves are asserted against Postgres in `auth.integration.test.ts`, and `e2e/sign-out.spec.ts` asserts that a request carrying the old cookie reads as `signed-out` from `GET /api/viewer`.

**It is a POST, and only a POST.** The control is a form's submit button in two places: inside the signed-in indicator's open list, and in the navbar's `<noscript>` beside the sign-in link, because the island never runs without JavaScript. The `<noscript>` form is shown to everybody, on the terms the sign-in link already is — the same markup for every visitor, reading nothing at render — so it adds nothing per-visitor to any page, the public Profile included. Next.js runs a server action only for a POST, so no link, image or redirect on another site can sign somebody out: `sign-out.spec.ts` sends GETs to the page, to the form's own address with its fields as a query string, and to `/api/auth/sign-out`, and the session survives every one.

**A cross-site POST is refused twice over.** Better Auth's session cookie is `SameSite=Lax`, so a browser does not attach it to a POST from another site; and Next.js's action handler aborts an action whose `Origin` host does not match the request's host. The spec posts the form's exact fields with `Origin: https://attacker.example` and the session survives, then posts the same request from the site's own origin and it ends — so the refusal is the origin check, not a malformed request. **A request with no `Origin` at all is let through** by Next.js with a warning: a browser sends one on every cross-site POST, and a handcrafted request cannot carry somebody else's cookie.

**Every answer lands on `/`.** The action takes no arguments and reads nothing from the form, so no field can choose where it goes. A failure — services that cannot be built, a sign-out that throws — is logged through `logFailure` as `sign_out_failed`, recorded as `failed` on the `sign-out.form` boundary line, and still lands on `/`, where the indicator asks again and shows what is true. The island dispatches `VIEWER_CHANGED_EVENT` once the request settles, so signing out on `/` itself — where the path does not change — shows the sign-in link without a reload. Nothing renders from the session, so there is no cached page for `revalidatePath` to clear.

**Better Auth reports success even when the row delete fails.** In better-auth 1.7.4's `sign-out` route a database error reading or deleting the session is logged by Better Auth and swallowed, and the cookie is expired regardless. That browser is signed out, but the row — and so a copy of the cookie held anywhere else — stays valid until the session expires. The action cannot see that case, so its boundary line says `redirected`.

### Deleting an account

An owner deletes their own Account from `/account`, which the signed-in indicator links to ([#195](https://github.com/joshstothard/3moji/issues/195)). Deleting an Account **is** the Release ([ADR-0004](../adr/0004-the-handle-model.md) decision 5, [ADR-0009](../adr/0009-release-leaves-a-tombstone-and-the-cooldown-is-dropped-for-the-mvp.md)): `deleteAccountAction` in `components/account-delete-action.ts` calls `releaseHandle` through the composition root's `releases` store, which writes the tombstone and deletes the `user` row in one transaction, and the Handle, sessions, Profile and Links go with it by cascade. The page says so in plain words before it offers anything: the Handle is given up, the Profile and its Links are deleted, and it cannot be undone.

- **Whose Account comes from the session only.** The action reads the user id from `readViewer()`; nothing in the form names an Account, so a posted `userId` is ignored. A request with no session is sent to `/sign-in` and deletes nothing.
- **The confirmation is a typed word the action checks.** The form's field must hold `AccountPage.confirmWord` (`delete`), compared after trimming and lower-casing. The page marks it `required`, but a direct POST ignores that, so the action refuses anything else with `/account?error=confirm`. A typed word rather than a second page, because a second page still ends in one POST anybody can send without visiting the first; a word rather than the Handle, because three emoji cannot be typed on a physical keyboard. It is a plain `<form>`, so it works without JavaScript.
- **Afterwards the person lands on `/` signed out.** The session row is already gone with the cascade, so the old cookie reads as signed out anywhere it is replayed. The action also calls Better Auth's `signOut`, which clears the cookie whether or not it finds a session row; a failure there is logged as `account_delete_sign_out_failed` and does not turn a committed deletion into an error. The Profile's path is revalidated before the redirect.
- **A failed Release deletes nothing**: the transaction rolls back, the failure is logged as `account_delete_failed` through `logFailure`, and the page says nothing was deleted. The Handle is claimable by anyone the moment the Release commits — there is no cooldown.

Only a signed-in owner can reach this, and only for their own Account. **A takedown still goes through the SQL** in `docs/runbooks/takedown.md` §3b.
...
### The header search's rate limit

**Search is limited per client address** ([ADR-0012](../adr/0012-header-search-lists-claimed-handles-with-display-names-capped-and-rate-limited.md) decision 6, [#254](https://github.com/joshstothard/3moji/issues/254)). `searchClientRateLimiter`, from `packages/core/src/handle/search-rate-limit.ts`, is asked by `GET /api/search` before the query is read. The search itself is described in [system-overview.md](system-overview.md#the-header-search).

| Limit              | Value                    | Constant                   |
| ------------------ | ------------------------ | -------------------------- |
| Per client address | 60 in a fixed 10 minutes | `SEARCH_CLIENT_RATE_LIMIT` |

- **The Claim's table, store and hashing**, under the bucket kind `search-client:`, grouped by client address like every other limiter: an IPv6 address by its `/64`, and every unreadable address sharing one `unknown` bucket. Its window fits inside `RATE_LIMIT_RETENTION_MS`. A new kind is a new prefix in a `text` column, so it needed no migration.
- **Every request counts**, an empty or unknown query included, so it is a limit on asking.
- **Over the limit is a plain `429`**, with no body and no `Retry-After`. A store that cannot count answers `500` and searches nothing: fail closed.
- **A starting value to tune**, like the Claim's. Five results a query and sixty queries per ten minutes bound how fast one address walks the claimed Handles; many addresses walk faster, which the ADR accepts. The island debounces typing by 250 ms, so somebody typing never meets it.
- **No response floor.** A search reveals only public Profile data, never whether an Account or an email address exists, so there is no timing to hide.
