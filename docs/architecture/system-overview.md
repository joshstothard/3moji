...
Pull requests merge themselves once CI passes ([ADR-0003](../adr/0003-auto-merge-pull-requests-on-green-ci.md)). `.github/workflows/auto-merge.yml` runs `scripts/auto-merge.mjs`, which merges a PR labelled `automerge`, or a Dependabot PR containing only minor and patch updates, when the latest CI run on its up-to-date head commit succeeded — the latest run that GitHub actually allowed to start, since a `GITHUB_TOKEN` push also produces a jobless run awaiting approval, which the gate reports as `ci-not-started` rather than as a failure ([#125](https://github.com/joshstothard/3moji/issues/125)). The repository has no GitHub branch protection (not available on its plan), so that workflow is the merge gate.
