...
- Node.js 24 (pinned in `.nvmrc`; run `nvm use`). The root `package.json` `engines` field accepts `^22.22.3 || ^24.15.0 || >=26.0.0` — the floor set by `@nestjs/schematics@12`. Older Node lines cannot load the ESM-only NestJS 12 packages from the CommonJS API.
