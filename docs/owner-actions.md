...
- [ ] **Clean up Neon preview branches**
  - **What:** The Neon project reached the Free plan's 10-branch limit on 2026-09-14, because the Vercel integration keeps each `preview/<git branch>` database branch for about six months. `.github/workflows/neon-preview-cleanup.yml` deletes them, once it has a key ([#258](https://github.com/joshstothard/3moji/issues/258)). After the pull request that adds it has merged:
    ...
    5. **Sweep the existing branches now.** In **Actions**, open **Neon Preview Cleanup**, select **Run workflow** on `main` (or run `gh workflow run neon-preview-cleanup.yml`), rather than waiting up to an hour for the scheduled sweep. The log names each branch it deleted, and how many preview branches had no open pull request. `main` and any branch without the `preview/` prefix are never touched. After this the workflow sweeps every hour by itself. - **Why it matters:** once the limit is reached, a new preview deployment gets no database branch, so previews can't be tested.
  - **How it keeps up:** a pull request closing deletes its branch only when it is merged by hand or closed without merging, because the auto-merge gate merges with `GITHUB_TOKEN`, which GitHub doesn't raise a `pull_request` event for. So the workflow also sweeps every hour, which cleans up after auto-merged pull requests, forks and Dependabot. It keeps any branch that could be a shortened name of an open pull request's branch ([CI/CD § Neon preview cleanup](development/ci-cd.md#neon-preview-cleanup)). How often it runs is decided (below).
  - **Options:** none left open. Do steps 1 to 5, or don't, and previews keep hitting the limit.
  - **Recommendation:** do steps 1 to 5 once the pull request has merged.
...
- [ ] **Review the privacy notice and terms before removing the draft marker**
  - **What:** `/privacy` and `/terms` show "Draft — pending owner review" until you remove the marker (`DraftMarker` in `apps/web/src/components/legal-document.tsx`). Answer these first. The ICO fee and the Online Safety Act are in the item above.
    ...
    2. ~~**Processor locations and transfers**~~ — filled from the providers' own pages in [#242](https://github.com/joshstothard/3moji/issues/242), and for Neon in [#257](https://github.com/joshstothard/3moji/issues/257). **Region:** you confirmed on 2026-09-14 that the production Neon project is in AWS US East 1 (N. Virginia), so the notice says Neon stores the database in the United States. **Safeguard:** the [DPF list](https://www.dataprivacyframework.gov/list) shows Databricks, Inc. active under the UK Extension, with Neon, LLC as a covered entity, as [Databricks' DPF notice](https://www.databricks.com/legal/dpf) says. The notice claims only that certification for Neon. It does not claim the UK addendum, because `neon.com/dpa` is now Databricks' Neon product schedule with no transfer clause, and the older [Neon DPA](https://neon.com/pdf/DPA.pdf) (Neon, Inc., 2024) has the Standard Contractual Clauses and UK addendum only as a fallback, and wasn't shown to apply to this account. **If you move the database to another region, update the notice.**
...
- **Neon preview branches are swept every hour** ([#258](https://github.com/joshstothard/3moji/issues/258)), decided by the orchestrator for you, since you asked for automatic cleanup. Almost every pull request merges through the auto-merge gate, which raises no close event, so a close trigger alone would almost never fire. The sweep runs at minute 23 of every hour, under the same `NEON_CLEANUP_ENABLED` switch. Setup is the "Clean up Neon preview branches" item above.
...
Accepted by the owner on 2026-09-14 ([#244](https://github.com/joshstothard/3moji/issues/244)), then recorded the same day in [ADR-0011](adr/0011-canonical-word-aliases-name-one-handle-and-unclaimed-aliases-list-claimable-handles.md) (PR [#249](https://github.com/joshstothard/3moji/pull/249)), which partially supersedes [ADR-0008](adr/0008-handles-are-addressable-by-emoji-and-by-their-word-alias.md). No `/adr` run is left for you. It was built in [#248](https://github.com/joshstothard/3moji/issues/248) (PR [#256](https://github.com/joshstothard/3moji/pull/256)):

- **Every canonical word address names exactly one Handle.** You accepted "prefer a shorter name where it matches only one Handle". ADR-0011 records a narrower version, after measuring the wider one: applied everywhere, it changed 156 of 307 addresses, often to the wrong word. So `displayName` slugs stay. Only the five display names that also name another emoji get a curated `aliasName`: 🦇 `bats`, 🐋 `whales`, 🦗 `grasshopper`, 🌼 `flower` and 🍨 `sundae`. 🍎🍎🍎 stays `red-apple.red-apple.red-apple`. Detail: ADR-0011 decisions 1 to 4.
- **A word address that matches several Handles, none of them claimed, lists the ones that can be claimed** (option 1 of [#121](https://github.com/joshstothard/3moji/issues/121)). Each row links to that Handle's claim call to action, and held and Reserved Handles are left out. Phase 4's alias criterion is now ticked. Detail: ADR-0011 decision 5.
