...
- 2026-09-14 — Neon preview database branches are deleted by `neon-preview-cleanup.yml`: on a pull request closing, and by an hourly scheduled sweep that keeps any `preview/` branch whose name is, or is the start of, an open pull request's head branch, in case Neon shortened it. The sweep is needed because the auto-merge gate's `GITHUB_TOKEN` merges raise no close event. The alternative, the gate dispatching the sweep after each merge, was not chosen. Decided by the orchestrator for the owner, who asked for automatic cleanup, after the Neon Free 10-branch limit was reached ([#258](https://github.com/joshstothard/3moji/issues/258)).
...
- 2026-09-14 — The emoji picker's category and emoji buttons get the `slate-500` border that the builder's slots have, rather than relying on the label or the glyph. Built in [#243](https://github.com/joshstothard/3moji/issues/243) (PR [#246](https://github.com/joshstothard/3moji/pull/246)). Accepted by the owner (#244).
...
- 2026-09-14 — The home page connects the builder, the picker and the claim form. On a wide screen it is one composer card (Option B, "One composer card"): the slot row sticks within the card and step 2, "Make it yours", unlocks at the bottom once the Handle is complete and available. On a phone both artboards: a sticky Handle bar while browsing, and a claim bottom sheet once the Handle is complete and available. Find a Handle moves below the composer until the header search (#254) lands. Decided by the repo owner ([#263](https://github.com/joshstothard/3moji/issues/263)).
- 2026-09-14 — On a phone, the way back to a dismissed claim sheet is a violet Claim pill in the sticky Handle bar. It replaces a floating reopen button fixed to the bottom of the screen, and dismissing the sheet returns focus to it. It shows only while the Handle is complete and available and the sheet is closed, and the bar keeps the same height either way. Wide screens are unchanged. Decided by the repo owner ([#272](https://github.com/joshstothard/3moji/issues/272), PR [#273](https://github.com/joshstothard/3moji/pull/273)).
- 2026-09-14 — The owner accepted that the shareable word address should prefer a shorter name where it names only one Handle. ADR-0011 (above) records the narrower form the owner chose after seeing the measurements, and [#248](https://github.com/joshstothard/3moji/issues/248) built it (PR [#256](https://github.com/joshstothard/3moji/pull/256)). Accepted by the owner (#244).
...
- 2026-09-14 — A word address that matches several Handles, none of them claimed, lists the ones that can be claimed (option 1 of [#121](https://github.com/joshstothard/3moji/issues/121)). ADR-0011 decision 5 records it, and #248 built it. Accepted by the owner (#244).
...
- 2026-09-14 — Recorded the owner's acceptance of the pre-launch recommendations ([#244](https://github.com/joshstothard/3moji/issues/244)): twelve Decision log lines, and the Open questions they settle removed (letters and shapes, three resends an hour, 🍑 and 🍆, the platform Reserved Handles). Decisions 9 and 11 point at ADR-0011 and its implementation, [#248](https://github.com/joshstothard/3moji/issues/248). The takedown runbook now states its response targets as a commitment.
- 2026-09-14 — ADR-0012 built (#254): the header search is in every page's header, a combobox over `GET /api/search` listing claimed Handles with their display names (5) and emoji (8), limited to 60 requests per client address per 10 minutes under `search-client:`. The home page's Find a Handle lookup is gone; a bare `/find` is now the lookup, the no-JavaScript fallback. The privacy notice says claimed Handles and display names can be found through the search. Architecture docs describe it as built.
