...
| **Safety and the law** | <ul><li>A privacy notice and terms of use, both drafts awaiting owner review.</li><li>A report link on Profiles, which appears once a contact address is set, plus a takedown runbook.</li><li>Rate limits on claiming, sign-in, password reset and search.</li><li>A nonce-based Content Security Policy and HSTS.</li><li>Analytics that redact tokens and query strings.</li></ul>                                                                                                                                |
| **Operations**         | <ul><li>Every deploy runs database migrations, and each preview gets its own database branch.</li><li>A nightly backup workflow that encrypts backups with age and stores them in R2. It's written but switched off until the owner creates the bucket and key.</li><li>A workflow that cleans up preview database branches. It's written but switched off until its API key is set.</li><li>Four runbooks: site down, email not arriving, restore from backup, takedown. Uptime checks are still to come.</li></ul> |
